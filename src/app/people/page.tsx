...
export default async function PeoplePage({
  ...
  function buildUrl(params: Record<string, string | undefined>) {
    ...
    const merged = { riskLabel, incidentType, year, page: undefined, ...params };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-zinc-900 mb-1">People Directory</h1>
        <p className="text-zinc-500">{total} documented individual{total !== 1 ? "s" : ""}</p>
      </div>
                ...
                <Card className="hover:shadow-md transition-shadow cursor-pointer h-full">
                  <CardContent className="pt-5 pb-5">
                    ...
                    {person.roles.length > 0 && (
                      <p className="text-xs text-zinc-500 mb-2">{person.roles.join(" · ")}</p>
                    )}
                    <p className="text-xs text-zinc-400">
                      {person._count.incidents} linked incident{person._count.incidents !== 1 ? "s" : ""}
                    </p>
                  </CardContent>
                </Card>
              </Link>
